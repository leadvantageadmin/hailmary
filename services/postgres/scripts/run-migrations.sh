...
if [ $MIGRATIONS_FAILED -eq 0 ]; then
    ...
    echo ""
    echo "🔧 Next steps:"
    echo "   • Check database schema: ./scripts/validate-schema.sh"
    echo "   • View migration history: docker compose exec postgres psql -U $POSTGRES_USER -d $POSTGRES_DB -c 'SELECT * FROM migration_status;'"
    ...
    echo "   • Check PostgreSQL logs: ./scripts/logs.sh"
